...
exports.inviteUserToBoard = async (req, res) => {
  ...
  try {
    ...
    // ── ส่ง Socket notification (in-app) ──
    try {
      req.app.get('io')
    ...
    try {
      const invitedUser = await User.scope('withPassword').findByPk(targetUser.id);
      if (invitedUser && invitedUser.line_user_id) {
        await sendLineMessage(
          invitedUser.line_user_id,
...
exports.respondToInvite = async (req, res) => {
  try {
    ...
    // ── ลบ notification board_invite ออกหลัง respond ──
    ...
    return res.json({ status: 'success', data: { board_id: boardId, action: status } });
  } catch (err) {
    if (err.name === 'SequelizeValidationError') {
      return res.status(400).json({
        status: 'error',
